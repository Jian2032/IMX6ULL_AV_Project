...
/*
 * av_lcdif_fb.c - i.MX6ULL LCDIF learning driver, round LCD-R4
 *
 * LCD-R4复用已经通过实机验证的R3点屏和fbdev代码，新增：
 *   1. 连续申请两帧RGB565 DMA显存，yres_virtual变为1200；
 *   2. 实现FBIOPAN_DISPLAY，通过NEXT_BUF在帧边界切换显存；
 *   3. 申请LCDIF中断，使用CUR_FRAME_DONE等待翻页真正生效；
 *   4. 实现标准FBIO_WAITFORVSYNC，供应用等待垂直同步边沿；
 *   5. fb_test增加移动色块双缓冲动画和VSYNC频率测试。
 *
 * R4仍然保持固定1024x600 RGB565模式，不支持动态改分辨率、像素
 * 格式、x方向平移或YWRAP。下一轮R5再做blank、统计和稳定性收尾。
 */
 ...
 * frame_size/fb_size：
 *   一帧是1024 * 600 * 2 = 1,228,800 bytes；R4连续申请两帧，
...
/*
 * R4仍然不允许改变可见分辨率、像素格式和xoffset；虚拟高度固定为
...
/*
 * 当前显示模式在probe时已经写入LCDIF，R4不允许动态改模式。因此
...
static irqreturn_t av_lcdif_irq_handler(int irq, void *data)
{
	...
	/*
	 * R4没有打开underflow/overflow中断，但若它们在其他事件发生时
	 * 已经留下状态，则顺便清除，避免以后使能诊断中断时收到旧事件。
	 */
...
static int av_lcdif_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "LCD-R4 probe begin\n");
	...
	dev_info(&pdev->dev,
		 "LCD-R4 ready: double buffering and frame sync are available\n");
	return 0;
...
static int av_lcdif_remove(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev,
		 "LCD-R4 remove complete: flips=%u, vsync_waits=%u\n",
...
MODULE_DESCRIPTION("i.MX6ULL LCDIF learning framebuffer driver - LCD-R4");
MODULE_LICENSE("GPL");
MODULE_VERSION("R4");
