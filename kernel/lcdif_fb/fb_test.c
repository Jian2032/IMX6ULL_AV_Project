/*
 * fb_test.c - LCD-R5 user-space framebuffer test
 *
 ...
 * LCD-R5再增加blank测试，验证停止和恢复LCDIF后显存内容仍然保留。
 */
...
#define DEFAULT_BLANK_SECONDS	2U
#define MAX_BLANK_SECONDS	60U
...
/*
 * FBIOBLANK参数直接传blank等级，而不是用户指针。测试先请求
 * POWERDOWN，等待指定秒数，再UNBLANK并额外等待一次VSYNC，证明
 * 控制器不只是接受了恢复命令，而是真的重新开始输出帧。
 *
 * SIGINT/SIGTERM处理函数只设置标志；ioctl放在普通进程上下文执行，
 * 避免在异步信号处理函数里调用不安全接口。
 */
static int run_blank_test(int fd, unsigned int seconds)
{
	...
	stop_requested = 0;
	old_sigint = signal(SIGINT, request_stop);
	old_sigterm = signal(SIGTERM, request_stop);

	printf("blanking LCD for %u second(s); Ctrl+C will restore it\n",
	       seconds);
	fflush(stdout);

	if (ioctl(fd, FBIOBLANK, FB_BLANK_POWERDOWN) < 0) {
		fprintf(stderr, "FBIOBLANK POWERDOWN failed: %s\n",
			strerror(errno));
	...
	if (ioctl(fd, FBIOBLANK, FB_BLANK_UNBLANK) < 0) {
		fprintf(stderr, "FBIOBLANK UNBLANK failed: %s\n",
			strerror(errno));
		goto restore_handlers;
	}
	...
	if (ioctl(fd, FBIO_WAITFORVSYNC, &crtc) < 0) {
		fprintf(stderr,
			"LCD unblanked but VSYNC verification failed: %s\n",
			strerror(errno));
		goto restore_handlers;
	}

	printf("blank test passed: LCD stopped and resumed with VSYNC\n");
	ret = stop_requested ? -1 : 0;
...
int main(int argc, char **argv)
{
	...
	int has_count_argument;
	int fd;
	int ret = EXIT_FAILURE;

	has_count_argument = strcmp(command, "flip") == 0 ||
			     strcmp(command, "vsync") == 0 ||
			     strcmp(command, "blank") == 0;

	if (has_count_argument) {
		if (strcmp(command, "blank") == 0)
			sync_count = DEFAULT_BLANK_SECONDS;
		...
		if (strcmp(command, "blank") == 0 &&
		    sync_count > MAX_BLANK_SECONDS) {
			fprintf(stderr,
				"blank seconds must be 1..%u\n",
				MAX_BLANK_SECONDS);
			return EXIT_FAILURE;
		}
		if (argc > 3)
	...
	if (strcmp(command, "blank") == 0) {
		ret = run_blank_test(fd, sync_count) == 0 ?
		      EXIT_SUCCESS : EXIT_FAILURE;
		goto close_fd;
	}
